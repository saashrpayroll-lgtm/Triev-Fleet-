...
import { XCircle, Clock, FileText, MessageSquare, Sparkles, Send, User, Phone, Wallet, Trash2, CheckSquare, Square, Search, Filter } from 'lucide-react';
...
const RequestManagement: React.FC = () => {
    ...
    const handleStatusFilterChange = (status: RequestStatus | 'all') => {
        ...
        if (['resolved', 'rejected'].includes(status)) {
            setActiveTab('history');
        } else if (['pending', 'in_progress', 'waiting_for_info'].includes(status)) {
            setActiveTab('active');
        }
    };
    ...
    // Filter Logic based on Search, Tabs, and Status Parameter
    const filteredRequests = requests.filter(req => {
        if (activeTab === 'trash') {
            ...
            if (filterStatus === 'all' && !matchesTab) return false;
        }
        ...
        if (searchTerm.trim()) {
            ...
            const ticketIdStr = req.ticketId ? req.ticketId.toString() : '';
            const matchesSearch =
                ticketIdStr.includes(term) ||
                (req.userName || '').toLowerCase().includes(term) ||
                (req.email || '').toLowerCase().includes(term) ||
                (req.subject || '').toLowerCase().includes(term) ||
                (req.description || '').toLowerCase().includes(term) ||
                (req.relatedEntityName || '').toLowerCase().includes(term);
            ...
            <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm">
                <div className="p-4 border-b border-border flex flex-col sm:flex-row gap-3 items-center justify-between bg-muted/20">
                    ...
                    <div className="relative w-full sm:max-w-md">
                        ...
                        <input
                            type="text"
                            placeholder="Search by Ticket ID, user, rider, or subject..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-9 pr-3 py-2 text-sm border border-input rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                        />
                    </div>
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        <select
                            ...
                            className="px-3 py-2 border border-input rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary/20 font-medium"
                        >
                            ...
                            {activeTab === 'trash' && <option value="deleted">Deleted (Trash)</option>}
                        </select>
                    </div>
                </div>
