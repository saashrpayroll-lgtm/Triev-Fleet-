...
import { X, Languages, Zap, Copy, Send, Loader2, MessageSquareText, Brain, Globe } from 'lucide-react';
...
const AIReminderModal: React.FC<AIReminderModalProps> = ({ rider, type, isOpen, onClose }) => {
    ...
    const [engineUsed, setEngineUsed] = useState<string | null>(null); // which AI engine generated this
    ...
    const generateMessage = async (lang: 'hindi' | 'english') => {
        ...
        try {
            ...
            // Detect which engine was used: Hindi tasks use Mistral if key set, else Gemini
            if (lang === 'hindi') {
                setEngineUsed('mistral'); // multilingual task type routes to Mistral first
            } else {
                setEngineUsed('gemini');  // analysis task type routes to Gemini first
            }
        } catch (error) {
        ...
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
            ...
            <div className="relative bg-card border border-border shadow-2xl rounded-3xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="p-6 pt-2 bg-muted/20 border-t border-border/50">
                    ...
                    <div className="relative bg-background rounded-2xl rounded-tl-sm p-4 border border-border shadow-sm min-h-[120px] mb-3">
                        {isGenerating ? (
                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 rounded-inherit backdrop-blur-[2px]">
                                ...
                                <span className="text-xs font-bold text-muted-foreground animate-pulse">
                                    {language === 'hindi' ? '🌐 Mistral → Gemini → Groq...' : '🧠 Gemini → Mistral → Groq...'}
                                </span>
                            </div>
                        ) : null}
                        <textarea
                    ...
                    {engineUsed && !isGenerating && message && (
                        <div className="flex items-center gap-1.5 mb-4">
                            <span className="text-[10px] text-muted-foreground">Generated by</span>
                            {engineUsed === 'mistral' && (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300">
                                    <Globe size={10} /> Mistral AI
                                </span>
                            )}
                            {engineUsed === 'gemini' && (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
