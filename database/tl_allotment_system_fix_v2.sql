-- TL ALLOTMENT & SUBMISSION SYSTEM (PATCH V2)
-- Fixes the timezone cast bug on date_on_sheet which caused the 1st of the month imports to be mapped to the previous month.
...
BEGIN
    ...
    collection_stats AS (
        ...
        WHERE wl.transaction_type IN ('DAILY_COLLECTION', 'RENT_COLLECTION', 'FTD_COLLECTION', 'COLLECTION')
          AND wl.mode = 'ADD'
          -- FIXED TIMEZONE CAST: Convert ISO string to TIMESTAMPTZ explicitly before extracting IST date
